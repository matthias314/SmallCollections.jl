...
Return an iterator over the weak_compositions of `n` of length `k`.
A weak omposition of `n` of length `k` is a `k`-tuple of non-negative integers that add up to `n`.
...
eltype(g::Type{<:Generator{WeakCompositions, <:Fix2{typeof(weak_composition)}}}) = eltype(WeakCompositions)
...
eltype(g::Type{<:Generator{WeakCompositions, <:Fix2{typeof(composition)}}}) = eltype(WeakCompositions)
