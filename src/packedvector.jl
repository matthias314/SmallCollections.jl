...
```
"""
struct PackedVector{U,M,T} <: AbstractVector{T}
    m::U
    n::PackedLength
    function _PackedVector{U,M,T}(m::U, n::Integer) where {U <: Unsigned, M, T <: Union{BitInteger,Bool,EmulatedInteger}}
        bitsize(T) < M && error(LazyString("type ", T, " has fewer than ", M, " bits"))
        bitsize(U) ÷ M <= typemax(PackedLength) || error(LazyString("capacity cannot exceed ", typemax(PackedLength)))
        new{U,M,T}(m, n % PackedLength)
    end
end
...
@propagate_inbounds function PackedVector{U,M,S}(v::AbstractSmallVector{N,T}) where {U,M,S,N, T <: Union{BitInteger, EmulatedInteger}}
    if bitsize(T) == M && (T <: Signed) == (S <: Signed)
        ...
        _PackedVector{U,M,S}(bits(v.b) % U, v.n)
    else
...
@inline function zeros(::Type{PackedVector{U,M,T}}, n::Integer) where {U,M,T}
    N = capacity(PackedVector{U,M,T})
    @boundscheck n <= N || error(LazyString("vector cannot have more than ", N, " elements"))
    _PackedVector{U,M,T}(zero(U), n)
end
...
@inline function ones(::Type{PackedVector{U,M,T}}, n::Integer) where {U,M,T}
    N = capacity(PackedVector{U,M,T})
    @boundscheck n <= N || error(LazyString("vector cannot have more than ", N, " elements"))
    mask = one(U) << unsigned(M*n) - one(U)
    m = all_ones(U, M) & mask
    _PackedVector{U,M,T}(m, n)
end
...
@inline function getindex(v::PackedVector{U,M,T}, s::SmallBitSet) where {U,M,T}
    @boundscheck checkbounds(v, s)
    if HAS_PEXT && M == 1
        m = pext(v.m, bits(s))
        _PackedVector{U,M,T}(m % U, length(s))
    else
        PackedVector{U,M,T}(@inbounds v[i] for i in s)
    end
end
...
@inline function deleteat(v::PackedVector{U,M,T}, i::Integer) where {U,M,T}
    ...
    m1 = v.m >> M & ~mask
    m2 = v.m & mask
    _PackedVector{U,M,T}(m1 | m2, v.n-1)
end
...
@inline function pop(v::PackedVector{U,M,T}) where {U,M,T}
    ...
    mask = unsafe_shl(one(U), s) - one(U)
    m = v.m & mask
    _PackedVector{U,M,T}(m, v.n-1), @inbounds v[v.n]
end

pushfirst(v::PackedVector) = v

@inline function pushfirst(v::PackedVector{U,M,T}, x) where {U,M,T}
    x = convert(T, x)
    @boundscheck begin
        checklength(v)
        checkvalue(M, x)
...
@inline function popfirst(v::PackedVector{U,M,T}) where {U,M,T}
    @boundscheck checkbounds(v, 1)
    _PackedVector{U,M,T}(v.m >> M, v.n-1), @inbounds v[1]
end
...
@inline function append(v::PackedVector{U,M,T}, w::PackedVector{W,M,T}) where {U,M,T,W}
...
function circshift(v::PackedVector{U,M,T}, k::Integer) where {U,M,T}
    ...
    _PackedVector{U,M,T}((unsafe_shl(v.m, M*k) | unsafe_lshr(v.m, M*length(v)-M*k)) & mask, v.n)
end
...
@inline function +(v::PackedVector{U,M,T}, w::PackedVector{U,M,T}) where {U,M,T}
...
"""
unsafe_add(v::PackedVector{U,M,T}, w::PackedVector{U,M,T}) where {U,M,T}  = _PackedVector{U,M,T}(v.m+w.m, v.n)
...
"""
unsafe_sub(v::PackedVector{U,M,T}, w::PackedVector{U,M,T}) where {U,M,T} = _PackedVector{U,M,T}(v.m-w.m, v.n)
...
function Random.rand(rng::AbstractRNG, ::SamplerType{PackedVector{U,M,T}}) where {U,M,T}
    ...
    mask = one(U) << unsigned(M*n) - one(U)
    _PackedVector{U,M,T}(rand(U) & mask, n)
end
