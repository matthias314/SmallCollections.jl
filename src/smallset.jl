...
"""
    empty(s::AbstractSmallSet{N,T}) where {N,T} -> AbstractSmallSet{N,T}
    empty(s::AbstractSmallSet{N,T}, U::Type) where {N,T,U} -> AbstractSmallSet{N,U}

Return an empty `AbstractSmallSet` with the same capacity as `s`,
and with element type changed to `U` if so specified.
The resulting dictionary is mutable if and only if `s` is so.
"""
