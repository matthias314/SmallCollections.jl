...
@testset "SmallSet" begin
    @test_inferred SmallSet([1,2,3]) SmallSet{UInt64}([1,2,3])
    for U in unsigned_types
        s = SmallSet{U}()
        @test_inferred isempty(s) true
        m = bitsize(U)
        @test_throws Exception SmallSet{U}([1, 2, 'x'])
        @test_throws Exception SmallSet{U}([1, 2, m+1])
        @test_throws Exception SmallSet{U}([0, 1, 2])
        t = Set{Int}(rand(1:m, rand(0:m)))
        s = @inferred SmallSet{U}(t)
        ...
        v = collect(Float32, t)
        @test_inferred SmallSet{U}(v) s
        s3 = @inferred SmallSet{UInt512}(s)
        @test s3 == s
...
@testset "SmallSet first/min etc" begin
    for U in unsigned_types
        m = bitsize(U)
...
@testset "SmallSet in/subset" begin
    for U in unsigned_types
        m = bitsize(U)
        t = Set{Int}(rand(1:m, rand(0:m)))
        s = @inferred SmallSet{U}(t)
...
@testset "SmallSet push/pop etc" begin
    for U in unsigned_types
        m = bitsize(U)
        t = Set{Int}(rand(1:m, rand(0:m)))
        s = @inferred SmallSet{U}(t)
        i = rand(1:m)
        @test_inferred push(s, i) push!(copy(t), i) SmallSet{U}
        @test_inferred push(s, Float32(i)) push!(copy(t), i) SmallSet{U}
        @test_throws Exception push(s, 0)
        @test_throws Exception push(s, m+1)
        @test_throws Exception push(s, 'x')
        @test_inferred push(s, 3, 4, 5, 6) push!(copy(t), 3, 4, 5, 6) SmallSet{U}
        if !isempty(t)
            i = maximum(t)
            @test_inferred pop(s) (delete!(copy(t), i), i) Tuple{SmallSet{U}, Int}
            i = rand(t)
            @test_inferred pop(s, i) (delete!(copy(t), i), i) Tuple{SmallSet{U}, Int}
            @test_inferred pop(s, Float64(i)) (delete!(copy(t), i), i) Tuple{SmallSet{U}, Float64}
            @test_inferred pop(s, 0, -1) (s, -1)
            @test_throws Exception pop(s, 0)
            @test_throws Exception pop(s, 'x')
            @test_inferred delete(s, i) delete!(copy(t), i) SmallSet{U}
...
@testset "SmallSet union etc" begin
    for U1 in unsigned_types, U2 in unsigned_types
        ...
        s1 = @inferred SmallSet{U1}(t1)
        t2 = Set{Int}(rand(1:m2, rand(0:m2)))
        s2 = @inferred SmallSet{U2}(t2)
        @test_inferred union(s1, s2) union(t1, t2) SmallSet{U}
        @test_inferred intersect(s1, s2) intersect(t1, t2) SmallSet{U1}
        @test_inferred intersect(s1, t2) intersect(t1, t2) SmallSet{U1}
        @test_inferred setdiff(s1, s2) setdiff(t1, t2) SmallSet{U1}
        @test_inferred setdiff(s1, t2) setdiff(t1, t2) SmallSet{U1}
        @test_inferred symdiff(s1, s2) symdiff(t1, t2) SmallSet{U}
    end
end
...
@testset failfast = true "SmallVector support" begin
    for N in (1, 2, 9, 16), T in test_types, m in (0, 1, round(Int, 0.7*N), N-1, N)
        ...
        u = rand(0:2, m)
        v = @inferred SmallVector{N,T}(u)
        @test_inferred support(v) Set{Int}(i for i in 1:m if u[i] != 0) SmallSet
    end
end
